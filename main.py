...
if args.cuda:
    model = nn.DataParallel(model) #model = nn.DataParallel(model,device_ids=[0,1])
    model.cuda()
...
def main():
    ...
    for epoch in range(1, args.epochs+1):
        print('This is %d-th epoch' %(epoch+15))
        total_train_loss = 0
        adjust_learning_rate(optimizer,epoch+15)
        ...
        print('epoch %d total training loss = %.3f' %(epoch+15, total_train_loss/len(TrainImgLoader)))
        
        ## Save ##
        savefilename = args.savemodel+'/checkpoint_'+str(epoch+15)+'.tar'
        torch.save({
                'epoch': epoch+15,
