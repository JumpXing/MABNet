 ...
 for dd in monkaa_dir:
   for im in os.listdir(monkaa_path+'/'+dd+'/left/'):
    if is_image_file(monkaa_path+'/'+dd+'/left/'+im):
     ...
     all_left_disp.append(monkaa_disp+'/'+dd+'/left/'+im[0:4]+'.pfm')
 ...
 for ss in subdir:
    ...
    for ff in flying:
      ...
      for im in imm_l:
       ...
       all_left_disp.append(flying_disp+'/TRAIN/'+ss+'/'+ff+'/left/'+im[0:4]+'.pfm')
 ...
 for ss in subdir:
    ...
    for ff in flying:
      ...
      for im in imm_l:
       ...
       test_left_disp.append(flying_disp+'/TEST/'+ss+'/'+ff+'/left/'+im[0:4]+'.pfm')
   ...
   for j in subdir2:
    for k in subdir3:
        ...
        for im in imm_l:
          ...
          all_left_disp.append(driving_disp+i+'/'+j+'/'+k+'/left/'+im[0:4]+'.pfm')
